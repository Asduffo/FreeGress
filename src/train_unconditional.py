...
from src.diffusion_model_unconditional import DiscreteDenoisingDiffusionUnconditional
...
def get_resume_adaptive(cfg, model_kwargs):
    ...
    if cfg.model.type == 'discrete':
        model = DiscreteDenoisingDiffusionUnconditional.load_from_checkpoint(resume_path, **model_kwargs)
    else:
...
def main(cfg: DictConfig):
    ...
    #we need to force these twos, otherwise the whole thing goes boom
    cfg.guidance.loss = "crossentropy"
    cfg.guidance.guidance_medium = "NONE"
    ...
    if cfg.model.type == 'discrete':
        model = DiscreteDenoisingDiffusionUnconditional(cfg=cfg, **model_kwargs)
    else:
