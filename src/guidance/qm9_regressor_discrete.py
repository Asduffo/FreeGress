...
class Qm9RegressorDiscrete(pl.LightningModule):
    ...
    def training_step(self, data, i):
        ...
        target = data.guidance.clone().to(data.guidance.device)
        data.y = torch.zeros(data.guidance.shape[0], 0).type_as(data.guidance)
    ...
    def validation_step(self, data, i):
        # input zero y to generate noised graphs
        target = data.guidance.clone().to(data.guidance.device)
        data.y = torch.zeros(data.y.shape[0], 0).type_as(data.guidance)
